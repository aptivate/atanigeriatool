...
class FastDispatchMixin(object):
    ...
    def copy_params(self, get_or_post, params):
        for key, value in params.iteritems():
            if hasattr(value, '__iter__'):
                get_or_post.setlist(key, value)
            else:
                get_or_post.setlist(key, [value])

    def get_fake_request(
            self, path, method='get', get_params=None,
        ...
        self.copy_params(request.GET, get_params)

        request.POST = request.POST.copy()
        self.copy_params(request.POST, post_params)
        ...
        request.current_page = self.default_page
    ...
    def fast_dispatch(
            self, view_name, method='get', url_args=None,
    ...
    def assert_not_redirected(
            self, response, expected_status_code=200, msg_prefix=''):
